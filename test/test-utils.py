...
class TestUtils(unittest.TestCase):
    ...
    def test_loss_MSE(self):
        ...
        ry = loss_f.partial(self.batch_vectors_1, 
                           self.batch_vectors_2, 
                           respect_to="y")
        self.assertTrue(np.array_equal(rx, self.batch_vectors_1/len(self.batch_vectors_1)),
                        msg=f"\n\033[31mError\033[0m\tExpected: {self.batch_vectors_1/3}\n\tGiven: {rx}")
        self.assertTrue(np.array_equal(ry, -self.batch_vectors_1/3),
                        msg=f"\n\033[31mError\033[0m\tExpected: {-self.batch_vectors_1/3}\n\tGiven: {ry}")
    ...
    def test_loss_CrossEntropy(self):
        ...
        expected = np.sum([-np.log(1e-13), 0])/2
        self.assertTrue(np.isclose(r,expected,rtol = 1e-12),
                        msg=f"\033[31mError\033[0m\tGiven: {r}; Expected {expected}")
